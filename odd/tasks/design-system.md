...
11. T11 — Follow-ups de advisories (3 slices) ✅ — hardening + specs + variante emerald [WU04, slice 4].
...
- **Slice 3 REVIEWED (medium → approved/burned)**: `review-17d288cd3b5b1cd7` (lens `review-reliability`, 5 paths / 97 líneas, base `6104eb1e`); acknowledged `sha256:ef721d8d`, authority burned. Advisory no bloqueante (follow-up): R3-NoSpecCoverageForCardBadgeRefactor (WARNING — sin spec que aserte host classes/variantes de appCard/appBadge), R3-BadgeBackgroundOwnershipSplit (WARNING — badge success de landing conserva `bg-emerald-600` junto a variant="success" mientras sus 3 hermanos no llevan clase), R3-RoadmapBadgeUtilityDuplicatesVariant (SUGGESTION — 7 badges del roadmap duplican bg-emerald-600 con variant success; se conservó bg para asegurar emisión CSS del color). FEATURE CERRADA (T1–T10, 3 slices revisados/quemados).
- **T11 (follow-ups advisories) PASS**: `f4a864f` (10 files: 6 ediciones + 4 specs nuevos). Hardening: badge/btn resuelven variantes desconocidas → neutral/primary en vez de crash (`class-utils` tolera `compute()` no-string; `icon-map` agrega la clave `secondary` documentada y hasta ahora inexistente). Variante `emerald` nueva (`bg-emerald-600 text-white`) + 8 usos migrados (1 landing + 7 roadmap) y eliminado el override `bg-emerald-600` — color idéntico, ownership único. Specs nuevos: badge (10), input (4), btn (5), card (3) → 33/33 en primitivas; suite completa 57 files/158 tests (exit 1 ambiental por 13 HttpErrorResponse a localhost:3000). Resolución de advisories: R3-StatusVariantUnguardedLookup ✅, R3-HasErrorContractUnverified ✅ (spec input), R3-HasErrorDirectiveContractUnverified ✅, R3-NoSpecCoverageForCardBadgeRefactor ✅, R3-NoTestCoverageForUiRefactor ✅ (parcial), R3-BadgeBackgroundOwnershipSplit ✅, R3-RoadmapBadgeUtilityDuplicatesVariant ✅, R3-ReportsDisabledAffordanceUnproved ✅ (ya cubierto por base classes de btn), R3-EventsStatusBadgeNotMigrated (se mantiene: clase dinámica por tipo de evento, no duplicación de utilidad). RDD assess → **medium**, 13 paths / **477 líneas** base `9f20cc9` → **slice 4 como PR chained** (usuario aprobó, 477 > 400 budget). Cierre de slice 4: START falló en preflight (`gentle-ai.review-integration.failure/v2` — `immutable_review_transport_unsupported`: el runtime OpenCode no es elegible para immutable receipt-review pese a haber revisado slices 2 y 3 hoy; retry_safe false; usuario eligió **no reportar** el defecto del proveedor); invocación declined capturada ejecutada y validada (`action: declined`, `consent: declined_this_candidate`, target `sha256:7cc41565`) → **slice 4 SIN revisión nativa, autoridad no creada**; entrega bajo policy ordinaria.
